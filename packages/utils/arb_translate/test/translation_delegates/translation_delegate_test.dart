...
class NoOpTranslationDelegate extends TranslationDelegate {
  const NoOpTranslationDelegate()
    : super(batchSize: 4096, context: null, useEscaping: false, relaxSyntax: false);
...
void main() {
  group('TranslationDelegate', () {
    final delegate = NoOpTranslationDelegate();

    test('validateResults returns true for resources without placeholders', () {
    ...
    test('validateResults returns true for resources with valid placeholders', () {
      final resources = {
      ...
      final results = {'key': 'value {count, plural, other{{count} items}}'};

      expect(delegate.validateResults(resources, results), isTrue);
    });

    test('validateResults returns false for resources with invalid placeholders', () {
      final resources = {
