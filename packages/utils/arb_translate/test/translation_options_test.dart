...
void main() {
  group('TranslateOptions', () {
    test('resolve returns options with values from argResults over yamlResults', () {
      const argResultsModelProvider = ModelProvider.vertexAi;
      const argResultsCustomModelProviderUrl = 'http://argResultsCustomModelProviderBaseUrl';
      ...
      const argResultsVertexAiProjectUrl = 'https://argResultsVertexAiProjectUrl/models';
      ...
      final yamlResults = TranslateYamlResults(
        ...
        customModelProviderBaseUrl: 'http://yamlResultsCustomModelProviderBaseUrl',
      ...
      expect(
        ...
        isA<TranslateOptions>()
            .having((options) => options.modelProvider, 'modelProvider', argResultsModelProvider)
            .having(
            ...
            .having((options) => options.customModel, 'customModel', argResultsCustomModel)
            ...
            .having((options) => options.disableSafety, 'disableSafety', argResultsDisableSafety)
            .having((options) => options.context, 'context', argResultsContext)
            .having((options) => options.excludeLocales, 'excludeLocales', argResultsExcludeLocales)
            .having((options) => options.batchSize, 'batchSize', argResultsBatchSize)
            ...
            .having((options) => options.useEscaping, 'useEscaping', argResultsUseEscaping)
            .having((options) => options.relaxSyntax, 'relaxSyntax', argResultsRelaxSyntax),
      );
    });
  });
}
