...
class TranslateArgParser {
  ...
  static const _customModelProviderBaseUrlKey = 'custom-model-provider-base-url';
  ...
  final _parser = ArgParser(usageLineLength: stdout.hasTerminal ? stdout.terminalColumns : null)
    ..addFlag(_helpKey, abbr: 'h', help: 'Print this usage information.', negatable: false)
    ..addOption(
      _modelProviderKey,
      ...
      allowedHelp: {for (final model in Model.values) model.key: model.name},
    )
    ..addOption(
      _modelKey,
    ...
    ..addOption(_apiKeyKey, help: 'API key used to make translation requests.')
    ..addOption(
      _vertexAiProjectUrlKey,
      help: 'The URL of the Vertex AI project to use for translation.',
    )
    ..addOption(_customModelProviderBaseUrlKey, help: 'The base URL of the custom model provider.')
    ..addFlag(
      _disableSafetyKey,
      help: 'Whether or not to disable content safety settings for translation.',
    )
  ...
  TranslateArgResults parse(List<String> args) {
    ...
    if (rawResults.rest.isNotEmpty) {
      throw FormatException('Unexpected positional argument "${rawResults.rest.first}".');
    }
    ...
    final model = rawResults.wasParsed(_modelKey)
        ? Model.values.firstWhereOrNull((model) => model.key == rawResults[_modelKey])
        : null;
    ...
    return TranslateArgResults(
      ...
      customModelProviderBaseUrl: rawResults[_customModelProviderBaseUrlKey] as String?,
      ...
      templateArbFile: rawResults[TranslateOptions.templateArbFileKey] as String?,
      useEscaping: _getBoolIfParsed(rawResults, TranslateOptions.useEscapingKey),
      relaxSyntax: _getBoolIfParsed(rawResults, TranslateOptions.relaxSyntaxKey),
    );
  }
