...
Schema-backed models are generated by `json_schema_gen` from const
`JsonSchema` definitions in the standalone `json_schema` package using
`@FromSchema`. The generated models have no Dart Edge dependency, so projects
can reuse the same Dart models and JSON Schemas across clients and servers.
